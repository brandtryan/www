function createSoccerViz() {
  ...
  .filter(function (el) { return el != "team" && el != "region" })
    ...
    .html(function(d) { return d })
      ...
      d3.selectAll("g.overallG").select("circle").transition().duration(1000).attr("r", function(p) { return radiusScale(p[datapoint])})

      }

      teamG.on("mouseover", highlightRegion)
      teamG.on("mouseout", function() {d3.selectAll("g.overallG").select("circle").style
        ("fill", "pink")})
